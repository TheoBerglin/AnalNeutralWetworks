...
def calculate_order_parameter(N, S, pattern):
    ...
    for i in range(N):
        sum += S[i]*pattern[i]
    return sum/N
def calculate_m(N, beta, feed_pattern, weights, time_max):
    m = list()
    org_pattern = feed_pattern.copy()
    for t in range(time_max):
        m.append(calculate_order_parameter(N, feed_pattern, org_pattern))
        async_update_index = random.randint(0, N-1)
        b_update = local_field(weights, feed_pattern, async_update_index)
        g_update = activation_function(b_update, beta)
        feed_pattern[async_update_index] = state_update(g_update)
    return m

def plot_order_paramter()
...
if __name__ == '__main__':
    ...
    m = calculate_m(N, beta, patterns[0], weights, time_max)
